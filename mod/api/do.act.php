...
if($dduser['id']>0){ //处于登陆状态
	...
	$row=$duoduo->select("user","*",'code="'.$web.'" and del=0 and openid="'.$webid.'"');
	...
	if($row['id']>0){
		...
		$duoduo->update('user', $set_con_arr, 'id="' . $row['id'].'"');

		user_login($row['id'],$row['ddpassword']);
...
//		}
	    ...
	    $input=array('webname'=>$webname,'openid'=>$webid,'img'=>$_SESSION["avatar"],'web'=>$web,'apireg'=>authcode(1,'ENCODE',DDKEY));
