	...
	case 'get_msg':
	    ...
	    if($dduser['id']>0){
			...
			if($dduser['id']==$info['uid'] || $dduser['id']==$info['senduser']){
	            ...
	            echo $msg='<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0" /><p style=" line-height:20px;">'.$duoduo->select('msg','content','id="'.$id.'"',2).'</p>';
			}
			else{
