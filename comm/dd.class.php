...
function set_constants($table) {
	...
	function sign(){
		...
		if($dduser['signtime']<$todaytime){
			//建添加
			$webset['sign']['jifenbao']=$this->level($dduser['id']);
	...
	function level($uid){
	 ...
	 $jfb=$this->select('mingxi','sum(jifenbao)','uid="'.$uid.'"and shijian in (2,13,8) order by id desc');
	...
	if ($jfb>0 and $jfb<1000 ){
		$jfb=rand(1,2);
		return  $jfb;
		exit();
		}	
	if ($jfb>=1000 and $jfb<5000 ){
		$jfb=rand(2,3);
		return  $jfb;
		exit();
		}	
	if ($jfb>=5000 and $jfb<10000 ){
		$jfb=4;
		return  $jfb;
		exit();
		}		
	if ($jfb>=10000 and $jfb<50000 ){
		$jfb=5;
		return  $jfb;
		exit();
		}	
	if ($jfb>=50000 and $jfb<100000 ){
		$jfb=10;
		return  $jfb;
		exit();
		}	
	...
	function register($device='pc'){
		...
		if (isset ($_POST['username']) && $_POST['username'] != '') {
			...
			if ($_SESSION['jian_username']!=trim($username))
			    jump(-1, '验证手机号码和注册手机号码不一致');
