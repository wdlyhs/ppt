...
* @version	2.1
*/
require_once DDROOT.'/q/vendor/autoload.php'; //引入加载文件 
use Qiniu\Auth;  
...
class pic_upload
{
	...
	* @var array
	*/
	var $allowed_file_ext = array( 'gif', 'jpg', 'jpeg', 'png','ipa','apk','pdf','txt','xls','doc','docx','mp4' );
	...
	* @var array
	*/
	var $image_ext        = array( 'gif', 'jpeg', 'jpg', 'jpe', 'png', 'txt');
	...
	*/
	...
	function upload_process()
	{
		...
		//-------------------------------------------------
		//这里用七牛云
		if ($webset['qiniu']['status']==1){
			
			
			
			$accessKey = $webset['qiniu']['accessKey'];  //秘钥
			$secretKey =  $webset['qiniu']['secretKey'];  //秘钥
			$auth = new Auth($accessKey, $secretKey);  //实例化
			$bucket=$webset['qiniu']['title'];
			$token = $auth->uploadToken($bucket);
			$uploadMgr = new UploadManager();
			$key=$this->resetname($FILE_NAME);
			
			$filePath=$_FILES[ $this->upload_form_field ]['tmp_name'];
			list($ret, $err) = $uploadMgr->putFile($token, $key, $filePath);  
			if ($err !== null) {  
				echo '上传失败';
			} else{
				
				$this->saved_upload_name=$webset['qiniu']["url"]."/".$key;
			}
