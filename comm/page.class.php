...
if (!function_exists('pageft')) {
	...
	function pageft1($totle, $displaypg = 20, $url = '',$wjt=0, $shownum = 1, $showtext = 1, $showselect = 0, $showlvtao = 7) {
		if(defined('ADMIN') && ADMIN==1 && $totle==0){
			return "暂无数据";
		}
		if(defined('ADMIN') && ADMIN==1){
		...
		if ($wjt == '1') {
			...
			if ($prepg)
				...
				$pagenav .= "<a href='javascript:;'>上一页</a>";
		}
		if ($shownum == 1) {
			$o = $showlvtao; //中间页码表总长度，为奇数
			$u = ceil($o / 2); //根据$o计算单侧页码宽度$u
	...
	$pagenav .='<span class="forumdisplay-list-pagechoice" style="border:0px"><select class="" id="jian_page">';
			for ($i = 1; $i <= $n; $i++) {
				$select="";
			    if ($i==$page)
				    $select='selected';
			
				$pagenav .='<option value="'.$url.'='.$i.'" '.$select.'>'.$i.'/'.$n.'</option>';
		...
		if ($wjt == '1') {
			...
			if ($nextpg)
				...
				$pagenav .= "<a href='javascript:;'>下一页</a>";
			
		}
		$pagenav .='<a href="#top" class="return-top"><i class="icon-up"></i></a>';
		if ($showselect == 1) {
			//下拉跳转列表，循环列出所有页码：	
			if($lastpg>=25555){
				$pagenav .= "跳至<select name='topage' size='1' onchange='window.location=\"$url=\"+this.value'>\n";
