...
function dd_position($duoduo,$cid){
	...
	$arr[]=array("id"=>$row['id'],"title"=>$row['title'],"jian_mod"=>$row1['mod'],"jian_act"=>$row1['act']);
	while ($row['pid']!=0){
	...
	$str=' <a href="/">'.首页.'</a> &gt'; 
	  foreach (array_reverse($arr) as $row){
               $str=$str.'<a href="'.u($row['jian_mod'],$row['jian_act'],array('cid'=>$row['id'])).'">'.$row['title'].'</a> &gt';     
	  }	
	return $str;
	}
