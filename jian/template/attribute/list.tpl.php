...
<table cellspacing="0" width="100%" style="border:1px  solid #DCEAF7; border-bottom:0px; background:#E9F2FB;">
        <tr>
              <td width="20%">&nbsp;<img src="images/arrow.gif" width="16" height="22" align="absmiddle" />&nbsp;<a href="<?=u(MOD,'addedi',array("cid"=>$_GET['cid']))?>" class="link3">[新增属性]</a> </strong></td>
					...
					<?php foreach ($row as $r){?>
					  <tr>
                        ...
                        <td><?=$r["title"]?></td>
                         ...
                         <td><?=$atypes[$r['input_type']]?></td>
                         <td><?=get_title(MOD,$r['aid'])?></td>
                         	<td class="input" field='sort' w='50' tableid="<?=$r["id"]?>" status='a' title="双击编辑"><?=$r["sort"]==DEFAULT_SORT?'——':$r["sort"]?></td>
						<td>
                        <?
                        if ($r['input_type']<>1){
						?>
                        <a href="<?=u("aitems",'list',array('aid'=>$r['id']))?>" class=link4>属性值</a>
                        
                        <? }?>
                        <a href="<?=u(MOD,'addedi',array('id'=>$r['id'],'cid'=>$r['cid']))?>" class=link4>修改</a></td>
					  </tr>
					<?php }?>
                  </table>
