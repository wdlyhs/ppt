...
if ($q){
	$where.=" and ddusername like '%$q%'";
}
if ($_GET['sid']){
	$where.=" and tjr=".$_GET['sid'];
}
...
if ($_GET["pid"]){
	  ...
	  $where.=" and b.pid=".$_GET["pid"];
	}

//$sql="select u.* from xia_user as u left join xia_browse as b  on u.id=b.uid where $where   group by u.id";
//
//$article=$duoduo->sql_to_array($sql);
//$total=count($article);
//
//$sql="select u.* from xia_user as u left join xia_browse as b  on u.id=b.uid where $where   group by u.id order by $by id desc limit $frmnum,$pagesize";
//
