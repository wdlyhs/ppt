...
function getCategoryList($id = 0, $level = 0,$page=0,$frmnum=20) {
	...
	global $frmnum;
	global $pagesize;
	$category_arr = $duoduo->select_all('type','*','pid="'.$id.'" and tag="'.$mod_tag.'" and title like "%'.$q.'%" order by sort desc,id asc limit '.$frmnum.','.$pagesize);
	...
	foreach ( $category_arr as $category ) {
		...
		if ($category['isshow']==0)
		    $name =$name."<font color='red'>(隐藏)</font>";
...
<tr>
  ...
  <td><a href='".u("attribute",'list',array('cid'=>$id))."'>属性列表</a> |<a href='".u(MOD,'addedi',array('id'=>$id,'do'=>'add'))."'>添加子分类</a> |&nbsp;<a href='".u($mod_tag,'addedi',array('cid'=>$id))."'>添加内容</a> |&nbsp;<a href='".u(MOD,'addedi',array('id'=>$id,'do'=>'edi'))."'>修改分类</a></td>
</tr> ";
